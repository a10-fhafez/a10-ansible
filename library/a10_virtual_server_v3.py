...
- name: Create a virtual server
  a10_virtual_server_v3:
    ...
    virtual_server_ports:
      ...
      - port-number: 443
        protocol: HTTPS
        template-tcp-proxy: "mytcpproxy"
        aflex-scripts: [{aflex: 'abc'}]
        no-auto-up-on-aflex: 0
        service-group: sg-80-tcp
        template-client-ssl: "myclientssltemplate"
        template-http: "httptempl"
      - port-number: 8090
        protocol: http
        service-group: sg-80-tcp
        template-http: "myhttptempl"
        template-persist-cookie: "mycookiepersistencetempl"
...
def main():
    ...
    argument_spec.update(
        dict(
            ...
            redistribution_flagged=dict(type='int', choices=[0,1,2], required=False, default=0),
    ...
    json_post = {
        'virtual-server': {
            'name': slb_virtual,
            'enable-disable-action': axapi_enabled_disabled(slb_virtual_status),
